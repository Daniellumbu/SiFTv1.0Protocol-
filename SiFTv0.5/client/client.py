...
from rsa_utility import encrypt_message  # Import the encryption function
...
if __name__ == '__main__':
    ...
    with open('plaintext_password.txt', 'wb') as f:
        f.write(password.encode('utf-8'))

    try:
        encrypt_message(
            pubkeyfile='test_pubkey.pem',  # Server's public key file
            plaintext_file='plaintext_password.txt',  # Password file
            output_file='encrypted_password.txt',  # Encrypted password file
            privkeyfile=None,  # No signing from client
            sign=False  # No signing
        )
    except Exception as e:
        print('Encryption Error for password:', str(e))
        sys.exit(1)
    ...
    try:
        loginp.handle_login_client(encrypted_username.decode("utf-8"),encrypted_password.decode("utf-8"))  # Pass the encrypted data to the login handler
