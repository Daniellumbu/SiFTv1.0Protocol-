...
class SiFT_LOGIN:
    ...
    def handle_login_client(self, username, password):
        ...
        client_random = Random.get_random_bytes(16)  # Generate 16 random bytes
        client_random_hex = client_random.hex() 
        final_transfer_key = client_random  # Set final transfer key
        timestamp = str(time.time_ns())
       
        msg_payload =   timestamp.encode('utf-8') + b'\n' + msg_payload + b'\n' + client_random_hex.encode('utf-8')
        ...
        # Verify request hash received in the login response
        if login_res_struct['request_hash'] != request_hash:
            raise SiFT_LOGIN_Error('Verification of login response failed')
        
        # Compute the final transfer key
      
        # Decode server_random from hexadecimal string to raw bytes
        server_random_bytes = bytes.fromhex(msg_payload[-32:].decode('utf-8'))

        # Compute the final transfer key
        initial_key_material = client_random + server_random_bytes  # Concatenate raw bytes
        final_transfer_key = HKDF(
            master=initial_key_material,
            key_len=32,
            salt=request_hash,
            hashmod=SHA256,
            num_keys=1
        ...
        # DEBUG
        if self.DEBUG:
            print(f"Derived Final Transfer Key: {final_transfer_key.hex()}")
        # DEBUG

        # Pass the final transfer key to the MTP protocol entity
