...
class SiFT_MTP:
	...
	def receive_msg(self):
		try:
			...
			with open('rcvstate.txt', 'rt') as sf:
				stored_sqn = int(sf.readline()[len("sqn: "):], base=10)  # Extract sequence number
		...
		received_sqn = int.from_bytes(header_sqn_field, byteorder='big')
		print("received",received_sqn)
		if received_sqn <= stored_sqn:
	...
	def send_msg(self, msg_type, msg_payload):
		...
		if isLoginReq:
			...
			with open("test_pubkey.pem", "rb") as key_file:
				public_key = key_file.read()
			public_key = RSA.import_key(public_key)
			rsa_cipher = PKCS1_OAEP.new(public_key)
			encrypted_key = rsa_cipher.encrypt(self.tk)
		...
		msg_length = header_length + payload_length + authtag_length + encrypted_key_length
		...
		if self.DEBUG:
			...
			print(f'Payload ({len(encrypted_payload)} bytes): {encrypted_payload.hex()}')
			print(f'AuthTag ({len(encrypted_key)} bytes): {encrypted_key.hex()}')
