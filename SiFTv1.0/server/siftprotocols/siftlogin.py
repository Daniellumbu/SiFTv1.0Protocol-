...
class SiFT_LOGIN:
    ...
    def handle_login_server(self):
        ...
        parts = msg_payload.split(b'\n')
        received_timestamp = int(parts[0].decode('utf-8')) / 1_000_000_000
        current_time = time.time_ns() / 1_000_000_000
     
# Convert nanoseconds to seconds
    
        print("the current time", current_time)
        if not (current_time - self.acceptance_window <= received_timestamp <= current_time + self.acceptance_window):
            raise SiFT_LOGIN_Error("Login request timestamp is outside the acceptable window.")

        client_random_bytes = bytes.fromhex(msg_payload[-32:].decode('utf-8'))
        ...
        server_random = Random.get_random_bytes(16)  # Generate server_random for login response
        server_random_hex = server_random.hex() 
        msg_payload = msg_payload + b'\n' + server_random_hex.encode('utf-8')

        initial_key_material = client_random_bytes + server_random  # Concatenate raw bytes
